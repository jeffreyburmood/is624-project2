...
# LM
set.seed(624)
fit.lm <- train(PH~., data=trainnew, method="lm", metric=metric,
                preProc=c("center", "scale"), trControl=trainControl)

# GLM
set.seed(624)
fit.glm <- train(PH~., data=trainnew, method="glm", metric=metric,
                 preProc=c("center", "scale"), trControl=trainControl)

# GLMNET
set.seed(624)
fit.glmnet <- train(PH~., data=trainnew, method="glmnet", metric=metric,
                    preProc=c("center", "scale"), trControl=trainControl)
...
# SVM
set.seed(624)
fit.svm <- train(PH~., data=trainnew, method="svmRadial", metric=metric,
                 preProc=c("center", "scale"), trControl=trainControl)

# CART
set.seed(624)
grid <- expand.grid(.cp=c(0, 0.05, 0.1))
fit.cart <- train(PH~., data=trainnew, method="rpart", metric=metric,
                  tuneGrid=grid, preProc=c("center", "scale"),
                  trControl=trainControl)
# KNN
set.seed(624)
fit.knn <- train(PH~., data=trainnew, method="knn", metric=metric,
                 preProc=c("center", "scale"), trControl=trainControl)
...
# Here's a comparison plot of the forecast values
# generate a prediction using the selected model and the test data
pred <- predict(lmTune,newdata=cma.imp.test.pred)
# generate a summary of how well the forecast values are
accuracy(pred,cma.imp.test$Yield)
# plot the forecast and the real values
x <- seq(1:nrow(cma.imp.test))
# the actual value are in green
plot(x=x, y=cma.imp.test$Yield,type="l",col="green")
...
We'll continue with a mroe in-depth analysis in the next section using these models.  
